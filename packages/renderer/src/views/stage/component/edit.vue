  ...
  >
    <el-scrollbar height="50vh">
      ...
      >
        ...
        <el-form-item label="场景名称" prop="displayName">
          <el-col :span="24">
        ...
        <el-form-item label="所属关卡" prop="section">
          ...
          >
            <el-option
              v-for="level in charLevels"
              :key="level.section"
              :label="level.section + ' ' + level.displayName"
              :value="level.section"
            >
            </el-option>
          </el-select>
        ...
        <el-form-item label="通道场景数量" prop="passageStageCount">
          <el-col :span="24">
            <el-input disabled v-model="ruleForm.passageStageCount"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="分支场景数量" prop="branchStageCount">
          <el-input disabled v-model="ruleForm.branchStageCount"></el-input>
        </el-form-item>
...
export default {
  ...
  setup() {
    ...
    type StageDialogState = {
      ...
      charLevels: level[] | never[];
    };
    const state: StageDialogState = reactive({
      isShowDialog: false,
      isEdit: false,
      ruleForm: {
        id: "场景ID",
        displayName: "场景名称",
        availableForCharacterId: "所属角色",
        section: "关卡层级",
        envModelId: "场景模型ID",
        mapId: "地图ID",
        minimapModelId: "场景类别",
        tagCode: "场景标签",
        randomWeight: "随机次数(关卡由场景随机组合)",
        fieldCode: "场景单属性集合",
        size: "场景大小",
        characterCountCode: "场景内普通单位数",
        initialCodePlotId: "场景初始化交互脚本ID",
        rewardGetterCode0: "物品奖励代码",
        rewardGetterCode1: "物品奖励代码2",
        portTeleporterCharacterId: "场景出入单位ID(传送门)",
        rewardMoney: "奖励原石数",
        rewardExp: "奖励经验值",
        musicId: "战斗背景音乐",
        safeMusicId: "通关背景音乐",
      ...
      charLevels: computed(() => {
        let levels: level[] = store.getters.getArrayData("level");
        return (
          (levels.map((level) => {
            return level.availableForCharacterId ==
    ...
    const initForm = () => {
      ...
      state.ruleForm.availableForCharacterId = "";
      state.ruleForm.passageStageCount = "";
      state.ruleForm.branchStageCount = "";
      state.ruleForm.notAllowRandomizeStages = "";
