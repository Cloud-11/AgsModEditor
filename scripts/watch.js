 ...
 * @returns {Promise<import('vite').RollupOutput|Array<import('vite').RollupOutput>|import('vite').RollupWatcher>}
 */
const setupServicesPackageWatcher = (viteDevServer) => {
  return getWatcher({
    name: "reload-page-on-services-package-change",
    configFile: "packages/services/vite.config.js",
    writeBundle() {
      viteDevServer.ws.send({
        type: "full-reload",
      });
...
(async () => {
  try {
    ...
    await setupServicesPackageWatcher(viteDevServer);
